...
# ---- Begin Asym Stuff (Maybe dump, it seems a little dumb

# Dilate the skeleton
dilate = pe.Node(fsl.ImageMaths(op_string="-thr %.1f -bin -dilF",
                                suffix="_dil"),
                 name="dilate")

# Flip the mean FA image
flipmeanfa = pe.Node(fsl.SwapDimensions(new_dims=("-x","y","z")),
                     name="flipmeanfa")

# Add the main and reversed mean FA images
addsymmeanfa = pe.Node(fsl.ImageMaths(op_string="-add",
                                      suffix="_add"),
                       name="addsymmeanfa")

# Divide the symmetrical mean FA image to preserve scale
divsymmeanfa = pe.Node(fsl.ImageMaths(op_string="-div 2",
                                      suffix="_div"),
                       name="divsymmeanfa")

# Skeletonize the symmetric mean FA image
makesymskel = pe.Node(fsl.TractSkeleton(skeleton_file=True),
                      name="makesymskel")

# Threshold the symmetric skeleton and mask it with the dilated original skeleton
threshsymskel = pe.Node(fsl.ImageMaths(op_string="-thr %.1f -mas"%skeleton_thresh,
                                       suffix="_thr"),
                        name="trheshsymskel")

# Flip the symmetrical skeleton
flipsymskel = pe.Node(fsl.SwapDimensions(new_dims=("-x","y","z")),
                      name="flipsymskel")

# Masked the symmetrical skeleton with its flipped image
masksymskel = pe.Node(fsl.ImageMaths(op_string="-mas",
                                     suffix="_sym"),
                      name="masksymskel")

# Binarize the now *really* symmetrical skeleton
makesymmask = pe.Node(fsl.ImageMaths(op_string="-bin",
                                         suffix="_mask"),
                          name="makesymskelmask")

# Make the (still weird to me) distance map mask
symdistmapmask = pe.Node(fsl.ImageMaths(in_file=brainmask,
                                        op_string="-mul -1 -add 1 -add"),
                         name="symdistmapmask")

# Create the symmetrical distance map
symdistancemap = pe.Node(fsl.DistanceMap(),
                         name="symdistancemap")

# Project FA data onto the symmetrical skeleton
projectsymfa = pe.Node(fsl.TractSkeleton(threshold=skeleton_thresh,
                                         use_cingulum_mask=True,
                                         project_data=True),
                       name="projectsymfa")

# Flip projected data
flipprojfa = pe.Node(fsl.SwapDimensions(new_dims=("-x","y","z")),
                     name="flipprojfa")

# Subtract the right from left hemi 
subprojfa = pe.Node(fsl.ImageMaths(op_string="-sub",
                                   suffix="_asym"),
                    name="subprojfa")

# Mask half the brain
fahemimask = pe.Node(fsl.ImageMaths(suffix="_hemi"),
                     name="fahemimask")
...
if args.r1:
    ...
    # Project R1 data onto the symmetrical skeleton
    projectsymr1 = pe.Node(fsl.TractSkeleton(threshold=skeleton_thresh,
                                             use_cingulum_mask=True,
                                             project_data=True),
                           name="projectsymr1")

    # Flip projected data
    flipprojr1 = pe.Node(fsl.SwapDimensions(new_dims=("-x","y","z")),
                         name="flipprojr1")

    # Subtract the right from left hemi 
    subprojr1 = pe.Node(fsl.ImageMaths(op_string="-sub",
                                       suffix="_asym"),
                        name="subprojr1")

    # Mask half the brain
    r1hemimask = pe.Node(fsl.ImageMaths(suffix="_hemi"),
                         name="r1hemimask")
...
def hemi_mask_opstring(filelist):
    """Return an fslmaths opstring to mask half of a 4D file depending on the length of a filelist."""
    return "-roi 91 91 0 218 0 182 0 %d"%len(check_subjects(filelist))
...
skeletor.connect([
    ...
    (makeskeleton,    dilate,          [("skeleton_file", "in_file")]),  
    (meanfa,          flipmeanfa,      [("out_file", "in_file")]),
    (meanfa,          addsymmeanfa,    [("out_file", "in_file")]),
    (flipmeanfa,      addsymmeanfa,    [("out_file", "in_file2")]),
    (addsymmeanfa,    divsymmeanfa,    [("out_file", "in_file")]),
    (divsymmeanfa,    makesymskel,     [("out_file", "in_file")]),
    (makesymskel,     threshsymskel,   [("skeleton_file", "in_file")]),
    (dilate,          threshsymskel,   [("out_file", "in_file2")]),
    (threshsymskel,   flipsymskel,     [("out_file", "in_file")]),
    (threshsymskel,   masksymskel,     [("out_file", "in_file")]),
    (flipsymskel,     masksymskel,     [("out_file", "in_file2")]),
    (masksymskel,     makesymmask,     [("out_file", "in_file")]),
    (makesymmask,     symdistmapmask,  [("out_file", "in_file2")]),
    (symdistmapmask,  symdistancemap,  [("out_file", "in_file")]),
    (divsymmeanfa,    projectsymfa,    [("out_file", "in_file")]),
    (symdistancemap,  projectsymfa,    [("distance_map", "distance_map")]),
    (mergefa,         projectsymfa,    [("merged_file", "data_file")]),
    (projectsymfa,    flipprojfa,      [("projected_data", "in_file")]),
    (projectsymfa,    subprojfa,       [("projected_data", "in_file")]),
    (flipprojfa,      subprojfa,       [("out_file", "in_file2")]),
    (skelgrabber,     fahemimask,      [(("fa_images", hemi_mask_opstring), "op_string")]),
    (subprojfa,       fahemimask,      [("out_file", "in_file")]),
    ...
    (fahemimask,      skeletonsink,    [("out_file", "@asym_projected_fa")]),
    ])
...
if args.r1:
    skeletor.connect([
        ...
        (divsymmeanfa,    projectsymr1,    [("out_file", "in_file")]),
        (symdistancemap,  projectsymr1,    [("distance_map", "distance_map")]),
        (mergefa,         projectsymr1,    [("merged_file", "data_file")]),
        (merger1,         projectsymr1,    [("merged_file", "alt_data_file")]),
        (projectsymr1,    flipprojr1,      [("projected_data", "in_file")]),
        (projectsymr1,    subprojr1,       [("projected_data", "in_file")]),
        (flipprojr1,      subprojr1,       [("out_file", "in_file2")]),
        (skelgrabber,     r1hemimask,      [(("r1_images", hemi_mask_opstring), "op_string")]),
        (subprojr1,       r1hemimask,      [("out_file", "in_file")]),
        (r1hemimask,      skeletonsink,    [("out_file", "@projected_r1_asym")]),
