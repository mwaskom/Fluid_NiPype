...
if exp.nruns > 1:
    firstlevel.connect([
     (preproc, fixed_fx, 
        [(("dilatemask.out_file", lambda x: x[0]), "flameo.mask_file"),
         (("meanfunc2.out_file", lambda x: x[0]), "overlayflame.background_image")]),
      (fsl_vol_model, fixed_fx,
        [(("selectcontrast.out", lambda x: [i[0] for i in x]), "copemerge.in_files"),
         (("selectcontrast.out", lambda x: [i[1] for i in x]), "varcopemerge.in_files"),
         (("selectcontrast.out", lambda x: len(x)), "l2model.num_copes")]),
         ])
...
firstlevel.connect([(preproc, mergesubs,
                       [(("volhighpass.out_file",fuf.sub,"preproc-vol_func.nii.gz"),"in1"),
                        ...
                        (("dilatemask.out_file",fuf.sub,"func_mask.nii.gz"),"in4")])
                    ])
